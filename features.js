...
(() => {
  ...
  /* ---------- ✔ FIXED FUNCTION — ONLY CHANGE IN THE ENTIRE FILE ---------- */
  function updateHeaderCounts(){
    ...
    qsa('.vc-header-actions .vc-count').forEach(span => {
      ...
      if (parent.classList.contains('vc-wish-btn')) {
        span.textContent = wish;      // ❤️ wishlist shows wishlist count
      } else {
        span.textContent = qty;       // 🛒 cart shows cart count
      }
    });
