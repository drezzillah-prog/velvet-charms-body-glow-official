...
async function notifySeller(req, { details, items, captureID, orderID, market, total }) {
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({ name: payerName, email: payerEmail === "Not provided" ? "" : payerEmail, message, paypal_order_id: orderID, paypal_capture_id: captureID, _subject: `PAID Body Glow order — ${orderID}` })
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const stored = parseStoredCart(orderDetails.purchase_units?.[0]?.custom_id);
    ...
    if (orderDetails.status === "COMPLETED") {
      ...
      const sellerNotificationSent = await notifySeller(req, { details: orderDetails, items, captureID: previousCapture.id, orderID: orderDetails.id || orderID, market: stored.market, total: expectedTotal });
    ...
    const response = await fetch(`${baseUrl}/v2/checkout/orders/${encodeURIComponent(orderID)}/capture`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", Prefer: "return=representation" } });
    ...
    const sellerNotificationSent = await notifySeller(req, { details: orderDetails, items, captureID, orderID: capture.id || orderID, market: stored.market, total: expectedTotal });
